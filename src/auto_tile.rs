...
impl<'a, 'b> AutoTiler<'a, 'b> {
	...
	fn try_add_request(&mut self, tile: TileObject, rule: AutoTileRule) {
		let tileset_id = tile.auto_tile.tileset_id();
		let tile_id = tile.auto_tile.id();
		if let Some(tileset) = self.tilesets.get(tileset_id) {
			if let Some(auto_name) = tileset.get_tile_name(tile_id) {
				if let Some(index) = tileset.get_auto_tile_index(auto_name, rule) {
	...
	} in requests.iter()
	{
		match *index {
			TileIndex::Standard(idx) => {
				if let Ok(ref mut tile) = query.get_component_mut::<Tile>(*entity) {
		...
		map_query.notify_chunk_for_tile(coord.pos, coord.map_id, coord.layer_id);
	}
}
